...
# Get data path to the ifcb data folder and your e-mail address
ifcb_path <- "Z:/data/ifcb139" # Base path containing folders: classifier, raw, manual and features etc

# Edit these
taxonomist <- "Wenche Eikrem"
taxonomist_email <- "wenche.eikrem@niva.no"
```

## Define Paths
Here, we define the file paths needed for exporting the labelled images. These paths include directories for storing MATLAB manual and raw IFCB data.

```{r define_paths}
# Define stable paths
features_folder <- file.path(ifcb_path, "features")
data_folder <- file.path(ifcb_path, "raw")
matlab_readme_file <- file.path("templates", "MATLAB-template.md")

# Initialize vectors to store paths for each year
manual_folders <- file.path(ifcb_path, "manual")
png_folders <- file.path(ifcb_path, "png_images", Sys.Date())

class2use_files <- file.path(ifcb_path, "classifier", "class2use_20250310.mat")
readme_files <- file.path("templates", "README_NIVA.md")
...
ifcb_extract_annotated_images(manual_folders,
                              class2use_files,
                              data_folder,
                              iconv(png_folders, from = "UTF-8", to = "latin1"),
                              skip_class = c("unclassified"),
                              verbose = FALSE,
                              overwrite = FALSE)
...
metadata <- ifcb_summarize_png_metadata(png_folders, 
                                        features_folder, 
                                        data_folder)

# List all .mat files in the class directory
mat_files <- list.files(path = manual_folders, pattern = "\\.mat$", full.names = TRUE)
...
taxa_names_clean <- gsub(" colony", "", taxa_names_clean)
taxa_names_clean <- gsub(" pair", "", taxa_names_clean)
taxa_names_clean <- gsub(" round", "", taxa_names_clean)
...
class_names <- data.frame(subfolder = taxa_names,
                          ...
                          worms_class = worms_records$class,
...
class_names %>%
  arrange(class_clean) %>%
  knitr::kable(caption = paste("Class Summary"))
```
...
metadata <- metadata %>%
  mutate(annotated_by = taxonomist,
         annotated_by_email = taxonomist_email,
         ship = "M/S Color Fantasy")

station_name <- NA
sample_name <- paste0("MS_FB_", metadata$sample)
depth <- 4
source <- "flowthrough"
...
ecotaxa_metadata <- ecotaxa_headers %>%
  mutate(
    ...
    object_annotation_hierarchy = metadata$class_clean,
    object_annotation_category = metadata$subfolder,
...
```{r write_tsvs}
for (i in seq_along(unique(ecotaxa_metadata$object_annotation_category))) {
  
  # Define path to subfolder
  subfolder_path <- file.path(png_folders,
                              unique(ecotaxa_metadata$object_annotation_category)[i])
  
  # Filter metadata for each class
  ecotaxa_metadata_ix <- ecotaxa_metadata %>%
    filter(object_annotation_category == unique(ecotaxa_metadata$object_annotation_category)[i]) %>%
    mutate(object_annotation_category = iRfcb:::truncate_folder_name(object_annotation_category))
  ...
  write_tsv(ecotaxa_metadata_ix,
            file.path(
              subfolder_path,
              paste0("ecotaxa_",
                     unique(iRfcb:::truncate_folder_name(ecotaxa_metadata$object_annotation_category))[i],
                     ".tsv")),
            na = "")
}
```
...
zip_name <- file.path(ecotaxa_folder, "ecotaxa_niva.zip")

# Create zip archive
ifcb_zip_pngs(png_folder = png_folders,
...
zip_filename <- file.path(figshare_folder, 
                          paste0("niva_ifcb_annotated_images.zip"))

ifcb_zip_pngs(png_folder = png_folders,
              zip_filename = zip_filename,
              readme_file = readme_files,
              email_address = taxonomist_email,
...
zip_filename <- file.path(figshare_folder, 
                          paste0("niva_ifcb_matlab_files.zip"))

ifcb_zip_matlab(manual_folder = manual_folders,
                features_folder = features_folder,
                class2use_file = class2use_files,
                zip_filename = zip_filename,
...
readme_content <- readLines("templates/README_NIVA_general.md", encoding = "UTF-8")

# Count images
counts <- ifcb_count_mat_annotations(manual_folders,
                                     class2use_files,
                                     skip_class = "unclassified")

# List all .mat files in the specified folder (excluding subfolders)
mat_files <- list.files(manual_folders, 
                        pattern = "\\.mat$", 
                        full.names = TRUE, 
                        recursive = FALSE)
...
readme_df <- data.frame(classes = nrow(counts),
                        images = sum(counts$n),
                        min_year = min_year,
                        max_year = max_year)
...
updated_readme <- gsub("<E-MAIL>", taxonomist_email, updated_readme)
updated_readme <- gsub("<YEAR>", year(Sys.Date()), updated_readme)
updated_readme <- gsub("<YEAR_START_NIVA>", readme_df$min_year, updated_readme)
updated_readme <- gsub("<YEAR_END_NIVA>", readme_df$max_year, updated_readme)
updated_readme <- gsub("<N_IMAGES_NIVA>", formatC(readme_df$images, format = "d", big.mark = ","), updated_readme)
updated_readme <- gsub("<CLASSES_NIVA>", readme_df$classes, updated_readme)

# Write the updated content back to the README.md file
writeLines(updated_readme, file.path(figshare_folder, "README.txt"), useBytes = TRUE)
...
runtime_values <- c(runtime_knit, runtime_extraction, runtime_summarize_metadata, runtime_copy_ecotaxa, runtime_zip_png, runtime_zip_matlab)
