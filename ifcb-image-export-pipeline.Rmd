...
knit: (function(inputFile, encoding) {
                        ...
                        ".html")) })
title: "`r paste('ifcb-image-export-pipeline:', params$classifier, 'classifiers')`"
---
...
NOTE: If you want a general README files included in the MANIFEST.txt, add it before running the create_manifest chunk.
...
## Extract PNG Images
Here all annotated images are extracted as .png images. The packaging of .pngs to the iRfcb archive is skipped.
...
for (classifier in seq_along(params$classifier[!grepl("iRfcb", params$classifier)])) {
  ...
  ifcb_zip_pngs(png_folder = png_folders[classifier],
                zip_filename = file.path(output_folder, paste0("smhi_ifcb_", tolower(params$classifier[classifier]), "_annotated_images.zip")),
...
for (classifier in seq_along(params$classifier)) {
  ...
  ifcb_zip_matlab(manual_folder = manual_folders[classifier],
                  ...
                  zip_filename = file.path(output_folder, paste0("smhi_ifcb_", tolower(params$classifier[classifier]), "_matlab_files.zip")),
...
## Create MANIFEST
This section creates the MANIFEST.txt for the entire output folder. Make sure to add a general README file to the output folder before executing this chunk, to have it included in the MANIFEST.
