...
for (classifier in seq_along(params$classifier[!grepl("iRfcb", params$classifier)])) {
  ...
  metadata <- metadata %>%
    mutate(annotated_by = ifelse(params$classifier[!grepl("iRfcb", params$classifier)][classifier] == "Tångesund", 
                                 "Malin Mohlin", 
                                 "Ann-Turi Skjevik"),
           annotated_by_email = ifelse(params$classifier[!grepl("iRfcb", params$classifier)][classifier] == "Tångesund", 
                                       mm_email, 
                                       ats_email),
...
for (classifier in seq_along(params$classifier[!grepl("iRfcb", params$classifier)])) {
  ...
  for (i in seq_along(unique(ecotaxa_metadata$object_annotation_hierarchy))) {
    ...
    subfolder_path <- file.path(png_folders[classifier], unique(ecotaxa_metadata$object_annotation_hierarchy)[i])
    ...
    write_tsv(ecotaxa_metadata_ix,
              file.path(subfolder_path, paste0("ecotaxa_", unique(iRfcb:::truncate_folder_name(ecotaxa_metadata$object_annotation_hierarchy))[i], ".tsv")),
              na = "")
  }
