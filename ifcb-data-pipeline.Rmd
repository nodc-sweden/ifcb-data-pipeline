...
date: "`r Sys.Date()`"
params: 
  classifier: "Baltic" # "Baltic", "Skagerrak-Kattegat", "Tangesund" or "Skagerrak-Kattegat-Tangesund"
  year: [2022] # each year separated by comma, e.g. [2022, 2023, 2024]
  ...
  regional: FALSE # If output should only contain data from the classifier region, e.g. Baltic data for the Baltic classifier
  multiyear_delivery: FALSE # Include all years in a single data delivery
...
humans <- list(
    ...
    "niva_smhi_baltic" = "Ann-Turi Skjevik, Malin Mohlin, Wenche Eikrem",
...
for (i in seq_along(params$year)) {
  ...
  if (nrow(missing_position) > 0) {
    ferrybox_positions <- tryCatch(
      ifcb_get_ferrybox_data(missing_position$timestamp, 
                             ferrybox_path, 
                             ship = "SveaAWS",
      ...
      error = function(e) {
        message(paste("No ferrybox match found for year", year, ":", e$message))
        NULL
      }
    )
    
    if (!is.null(ferrybox_positions)) {
      hdr_data <- hdr_data %>%
...
for (i in seq_along(params$year)) {
  ...
  data_aggregated_year <- data_aggregated_year %>%
    filter(!str_detect(pid, blacklist_pattern))
  ...
  shark_col <- shark_col[rep(NA_integer_, nrow(data_aggregated_year)), ]
  ...
  shark_df <- shark_col %>%
    mutate(MYEAR = data_aggregated_year$year,
           ...
           SLABO = "SMHI",
           ACKR_SMP = "N",
           SMTYP = ifelse(params$classifier == "Tangesund", "IFCB", "FerryBox"),
           SMVOL = round(data_aggregated_year$ml_analyzed, 3), # VOLUME
           SMPNO = data_aggregated_year$pid, # SAMPLE NAME
           LATNM = data_aggregated_year$scientificname, # SPECIES
  ...
  # Remove samples that do not have ferrybox data (i.e. pump was probably turned off)
  if (!params$classifier == "Tangesund") {
    shark_df <- shark_df %>%
      filter(!is.na(WATERFLOW_FB))
  }
