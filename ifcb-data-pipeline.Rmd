...
date: "`r Sys.Date()`"
params: 
  classifier: "Skagerrak-Kattegat" # "Baltic", "Skagerrak-Kattegat" or "Tångesund"
...
for (i in seq_along(params$year)) {
  ...
  if (params$classifier == "Tångesund") {
    
    # Keep only relevant information and add position to Tångesund
  ...
  if(!params$classifier == "Tångesund") {
    ...
    # Create a data frame with one row filled with NA values for Tångesund data
...
for (i in seq_along(params$year)) {
  ...
  classifier <- gsub("\xe5", "å", classifier, useBytes = TRUE)
...
for (i in seq_along(params$year)) {
  ...
  unclassified <- data_aggregated_year %>%
    ...
    select(sample, classifier, counts, ml_analyzed, counts_per_liter, biovolume_mm3_per_liter, depth, timestamp,
           date, year, month, day, time, ifcb_number, platform, flag, gpsLatitude, gpsLongitude, near_land, in_baltic, 
           basin, analysis_date, cruise_number, verification,
           classifier_created_by, classifier_used, metoa) %>%
  ...
  shark_df <- shark_col %>%
    mutate(MYEAR = data_aggregated_year$year,
           ...
           ORDERER = "SMHI",
           SHIPC = ifelse(params$classifier == "Tångesund", NA, "77SE"),
           ...
           POSYS = "GPS",
           PDMET = ifelse(params$classifier == "Tångesund", "SEP", "MIX"),
           METFP = "None",
           ...
           SLABO = "SMHI",
           ACKR_SMP = "N",
           SMTYP = ifelse(params$classifier == "Tångesund", NA, "FerryBox"),
           ...
           METDC = paste("https://github.com/hsosik/ifcb-analysis",
                         ...
                         sep = ", "), # METHOD
           TRAINING_SET_ANNOTATED_BY = ifelse(params$classifier == "Tångesund", "Malin Mohlin", "Ann-Turi Skjevik"),
    ...
    mutate(
      ...
      MANUAL_QC_DATE = ifelse(if_any(c(PH_FB, CHL_FB, CDOM_FB, PHYC_FB, PHER_FB, WATERFLOW_FB, TURB_FB,
                                       PCO2_FB, TEMP_FB, PSAL_FB, OSAT_FB, DOXY_FB), ~ !is.na(.x)),
                              NA, MANUAL_QC_DATE),
      TRAINING_SET_ANNOTATED_BY = ifelse(if_any(c(PH_FB, CHL_FB, CDOM_FB, PHYC_FB, PHER_FB, WATERFLOW_FB, TURB_FB,
                                                  PCO2_FB, TEMP_FB, PSAL_FB, OSAT_FB, DOXY_FB), ~ !is.na(.x)),
                                         NA, TRAINING_SET_ANNOTATED_BY),
      ...
      METDC = ifelse(if_any(c(PH_FB, CHL_FB, CDOM_FB, PHYC_FB, PHER_FB, WATERFLOW_FB, TURB_FB,
                              PCO2_FB, TEMP_FB, PSAL_FB, OSAT_FB, DOXY_FB), ~ !is.na(.x)),
                     NA, METDC),
      COUNTPROG = ifelse(if_any(c(PH_FB, CHL_FB, CDOM_FB, PHYC_FB, PHER_FB, WATERFLOW_FB, TURB_FB,
                                  PCO2_FB, TEMP_FB, PSAL_FB, OSAT_FB, DOXY_FB), ~ !is.na(.x)),
                         NA, COUNTPROG),
